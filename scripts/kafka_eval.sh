...
docker exec -i $(docker ps -f "name=kafka" -q) bash -c "/opt/bitnami/kafka/bin/kafka-console-consumer.sh --topic flight_delay_classification_response --bootstrap-server kafka:9092 --from-beginning --max-messages 50"
